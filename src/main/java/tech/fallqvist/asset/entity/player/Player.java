...
public class Player extends Entity {
	...
	public void getAttackImages() {
		if (getCurrentWeapon() instanceof OBJ_Sword_Normal) {
			setAttackUp1(setup("/images/player/boy_attack_up_1", getGamePanel().getTileSize(),
					getGamePanel().getTileSize() * 2));
			setAttackUp2(setup("/images/player/boy_attack_up_2", getGamePanel().getTileSize(),
					getGamePanel().getTileSize() * 2));
			setAttackDown1(setup("/images/player/boy_attack_down_1", getGamePanel().getTileSize(),
					getGamePanel().getTileSize() * 2));
			setAttackDown2(setup("/images/player/boy_attack_down_2", getGamePanel().getTileSize(),
					getGamePanel().getTileSize() * 2));
			setAttackLeft1(setup("/images/player/boy_attack_left_1", getGamePanel().getTileSize() * 2,
					getGamePanel().getTileSize()));
			setAttackLeft2(setup("/images/player/boy_attack_left_2", getGamePanel().getTileSize() * 2,
					getGamePanel().getTileSize()));
			setAttackRight1(setup("/images/player/boy_attack_right_1", getGamePanel().getTileSize() * 2,
					getGamePanel().getTileSize()));
			setAttackRight2(setup("/images/player/boy_attack_right_2", getGamePanel().getTileSize() * 2,
					getGamePanel().getTileSize()));
		}

		if (getCurrentWeapon() instanceof OBJ_Axe) {
			setAttackUp1(setup("/images/player/boy_axe_up_1", getGamePanel().getTileSize(),
					getGamePanel().getTileSize() * 2));
			setAttackUp2(setup("/images/player/boy_axe_up_2", getGamePanel().getTileSize(),
					getGamePanel().getTileSize() * 2));
			setAttackDown1(setup("/images/player/boy_axe_down_1", getGamePanel().getTileSize(),
					getGamePanel().getTileSize() * 2));
			setAttackDown2(setup("/images/player/boy_axe_down_2", getGamePanel().getTileSize(),
					getGamePanel().getTileSize() * 2));
			setAttackLeft1(setup("/images/player/boy_axe_left_1", getGamePanel().getTileSize() * 2,
					getGamePanel().getTileSize()));
			setAttackLeft2(setup("/images/player/boy_axe_left_2", getGamePanel().getTileSize() * 2,
					getGamePanel().getTileSize()));
			setAttackRight1(setup("/images/player/boy_axe_right_1", getGamePanel().getTileSize() * 2,
					getGamePanel().getTileSize()));
			setAttackRight2(setup("/images/player/boy_axe_right_2", getGamePanel().getTileSize() * 2,
					getGamePanel().getTileSize()));
		}
	}
	...
	public void update() {
		if (isAttacking()) {
			attacking();
				...
				|| keyHandler.isRightPressed() || keyHandler.isEnterPressed() || keyHandler.isSpacePressed()) {
	...
	private void attacking() {
		...
		if (getSpriteCounter() > 5 && getSpriteCounter() <= 25) {
			...
			int interactiveTileIndex = getGamePanel().getCollisionChecker().checkEntity(this,
					getGamePanel().getInteractiveTiles());
		...
		if (index != 999) {
			if (!getGamePanel().getMonsters()[getGamePanel().getCurrentMap()][index].isInvincible()) {
				...
				int damage = attackPower
						- getGamePanel().getMonsters()[getGamePanel().getCurrentMap()][index].getDefensePower();
				if (damage < 0) {
					damage = 0;
				}

				getGamePanel().getMonsters()[getGamePanel().getCurrentMap()][index].setCurrentLife(
						getGamePanel().getMonsters()[getGamePanel().getCurrentMap()][index].getCurrentLife() - damage);
				...
				if (getGamePanel().getMonsters()[getGamePanel().getCurrentMap()][index].getCurrentLife() <= 0) {
					...
					getGamePanel().getUi().addMessage("Killed the "
							+ getGamePanel().getMonsters()[getGamePanel().getCurrentMap()][index].getName() + "!");
					setExp(getExp() + getGamePanel().getMonsters()[getGamePanel().getCurrentMap()][index].getExp());
					getGamePanel().getUi().addMessage(
							"Exp + " + getGamePanel().getMonsters()[getGamePanel().getCurrentMap()][index].getExp());
	...
	private void damageInteractiveTile(int index) {
		if (index != 999 && getGamePanel().getInteractiveTiles()[getGamePanel().getCurrentMap()][index].isDestructible()
				&& getGamePanel().getInteractiveTiles()[getGamePanel().getCurrentMap()][index]
						.isCorrectWeapon(getCurrentWeapon())
			...
			getGamePanel().getInteractiveTiles()[getGamePanel().getCurrentMap()][index].setCurrentLife(
					getGamePanel().getInteractiveTiles()[getGamePanel().getCurrentMap()][index].getCurrentLife() - 1);
			getGamePanel().getInteractiveTiles()[getGamePanel().getCurrentMap()][index].setInvincible(true);

			generateParticle(getGamePanel().getInteractiveTiles()[getGamePanel().getCurrentMap()][index],
					getGamePanel().getInteractiveTiles()[getGamePanel().getCurrentMap()][index]);

			if (getGamePanel().getInteractiveTiles()[getGamePanel().getCurrentMap()][index].getCurrentLife() == 0) {
				getGamePanel().getInteractiveTiles()[getGamePanel().getCurrentMap()][index] = getGamePanel()
						.getInteractiveTiles()[getGamePanel().getCurrentMap()][index].getDestroyedForm();
			}
		}
	}
	...
	private void interactWithMonster(int index) {
		if (index != 999) {
			if (!isInvincible() && !getGamePanel().getMonsters()[getGamePanel().getCurrentMap()][index].isDying()) {
				...
				int damage = getGamePanel().getMonsters()[getGamePanel().getCurrentMap()][index].getAttackPower()
						- getDefensePower();
				if (damage < 0) {
					damage = 0;
				}
	...
	public void selectItem() {
		int itemIndex = getGamePanel().getUi().getItemIndexFromSlot(getGamePanel().getUi().getPlayerSlotCol(),
				getGamePanel().getUi().getPlayerSlotRow());
