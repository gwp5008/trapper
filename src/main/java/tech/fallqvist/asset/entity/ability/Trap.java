...
public abstract class Trap extends Entity {
	...
	public void set(int[] worldX, int[] worldY, boolean alive, Entity user) {
		this.setWorldXPoints(worldX);
		this.setWorldYPoints(worldY);
		this.setAlive(alive);
		this.user = user;
	...
	public void update() {
		...
		if (user == getGamePanel().getPlayer()) {
			...
			if (monsterIndex != 999) {
				...
				generateParticle(user.getProjectile(),
						getGamePanel().getMonsters()[getGamePanel().getCurrentMap()][monsterIndex]);
				setAlive(false);
			}
