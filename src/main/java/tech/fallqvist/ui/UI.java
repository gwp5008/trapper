...
public class UI {
	...
	private void drawStartScreen() {
		...
		graphics2D.drawImage(gamePanel.getPlayer().getDown1(), x, y, gamePanel.getTileSize() * 2,
	...
	private void drawCurrentEquipment(int textY, int tailX) {
		graphics2D.drawImage(gamePanel.getPlayer().getCurrentWeapon().getImage1(), tailX - gamePanel.getTileSize(),
				textY - 14, null);
		textY += gamePanel.getTileSize();

		graphics2D.drawImage(gamePanel.getPlayer().getCurrentShield().getImage1(), tailX - gamePanel.getTileSize(),
				textY - 14, null);
	}
	...
	private void drawInventoryScreen(Entity entity, boolean cursor) {
		...
		if (cursor) {
			...
			drawItemDescriptionText(inventory, descriptionFrameX, descriptionFrameY, descriptionFrameWidth,
					descriptionFrameHeight, slotCol, slotRow);
		}
	}

	private void drawItemsInInventory(Entity entity, int slotXStart, int slotX, int slotY, int slotSize,
			List<Asset> inventory) {
		for (int i = 0; i < inventory.size(); i++) {
			Asset object = inventory.get(i);

			// EQUIPPED BOX COLOR
			if (object == entity.getCurrentWeapon() || object == entity.getCurrentShield()) {
	...
	private void drawItemDescriptionText(List<Asset> inventory, int descriptionFrameX, int descriptionFrameY,
			int descriptionFrameWidth, int descriptionFrameHeight, int slotCol, int slotRow) {
		...
		if (itemIndex < npc.getInventory().size()) {
			...
			if (gamePanel.getKeyHandler().isEnterPressed()) {
				if (npc.getInventory().get(itemIndex).getPrice() > gamePanel.getPlayer().getCoins()) {
					...
					gamePanel.getPlayer()
							.setCoins(gamePanel.getPlayer().getCoins() - npc.getInventory().get(itemIndex).getPrice());
		...
		if (itemIndex < gamePanel.getPlayer().getInventory().size()) {
			...
			if (gamePanel.getKeyHandler().isEnterPressed()) {
				if (gamePanel.getPlayer().getInventory().get(itemIndex) == gamePanel.getPlayer().getCurrentWeapon()
						|| gamePanel.getPlayer().getInventory().get(itemIndex) == gamePanel.getPlayer()
								.getCurrentShield()) {
					commandNumber = 0;
					subState = 0;
					gamePanel.setGameState(gamePanel.getDialogueState());
